...
void classColorPicker::_createColorPicker(void)
{
  ...
  lv_obj_set_style_bg_color(_ovlPanel, lv_color_lighten(colorBg, 70), LV_PART_MAIN | LV_STATE_DEFAULT);
  ...
  lv_obj_set_size(_panelRGB, SCREEN_WIDTH, 250);
  lv_obj_set_align(_panelRGB, LV_ALIGN_TOP_MID);
  lv_obj_clear_flag(_panelRGB, LV_OBJ_FLAG_SCROLLABLE);
  lv_obj_set_style_bg_opa(_panelRGB, 0, LV_PART_MAIN | LV_STATE_DEFAULT);

  // lower part / cct
  _panelCCT = lv_obj_create(_ovlPanel);
  lv_obj_remove_style_all(_panelCCT);
  lv_obj_set_size(_panelCCT, SCREEN_WIDTH, 180);
  lv_obj_align(_panelCCT, LV_ALIGN_TOP_MID, 0, 250);
  lv_obj_clear_flag(_panelCCT, LV_OBJ_FLAG_SCROLLABLE);
  lv_obj_set_style_bg_opa(_panelCCT, 0, LV_PART_MAIN | LV_STATE_DEFAULT);

  // back button closes overlay
  _btnExit = lv_btn_create(_ovlPanel);
  lv_obj_set_size(_btnExit, 80, 40);
  lv_obj_align(_btnExit, LV_ALIGN_BOTTOM_LEFT, 15, -15);
  ...
  _colorWheel = lv_colorwheel_create(_panelRGB, true);
  ...
  _labelBrightness = lv_label_create(_panelCCT);
  lv_obj_set_size(_labelBrightness, LV_SIZE_CONTENT, LV_SIZE_CONTENT);
  lv_label_set_text(_labelBrightness, "100 %");
  lv_obj_align(_labelBrightness, LV_ALIGN_TOP_RIGHT, -20, 20);

  _labelColorTemp = lv_label_create(_panelCCT);
  lv_obj_set_size(_labelColorTemp, LV_SIZE_CONTENT, LV_SIZE_CONTENT);
  lv_label_set_text(_labelColorTemp, "4000 K");
  lv_obj_align(_labelColorTemp, LV_ALIGN_TOP_RIGHT, -20, 100);

  // _barBrightness
  _barBrightness = lv_bar_create(_panelCCT);
  lv_bar_set_range(_barBrightness, 0, 100);

  lv_obj_set_size(_barBrightness, 290, 40);
  lv_obj_align(_barBrightness, LV_ALIGN_TOP_LEFT, 15, 40);
  ...
  // _sliderBrightness
  _sliderBrightness = lv_slider_create(_panelCCT);
  lv_slider_set_range(_sliderBrightness, 0, 100);

  lv_obj_set_size(_sliderBrightness, 260, 34);
  lv_obj_align(_sliderBrightness, LV_ALIGN_TOP_LEFT, 30, 43);
  ...
  // ct bar lower
  _barCTlow = lv_bar_create(_panelCCT);
  lv_bar_set_range(_barCTlow, 0, 100);

  lv_obj_set_size(_barCTlow, 163, 40);
  lv_obj_align(_barCTlow, LV_ALIGN_TOP_LEFT, 15, 120);
  ...
  // ct bar upper
  _barCThigh = lv_bar_create(_panelCCT);
  lv_bar_set_range(_barCThigh, 0, 100);

  lv_obj_set_size(_barCThigh, 163, 40);
  lv_obj_align(_barCThigh, LV_ALIGN_TOP_RIGHT, -15, 120);
  ...
  // _sliderCT
  _sliderCT = lv_slider_create(_panelCCT);
  lv_slider_set_range(_sliderCT, 2000, 6000);

  lv_obj_set_size(_sliderCT, 260, 34);
  lv_obj_align(_sliderCT, LV_ALIGN_TOP_LEFT, 30, 123);
...
classColorPicker::classColorPicker(classTile *tile, lv_event_cb_t colorPickerEventHandler, bool enRGB, bool enCCT)
{
  ...
  // hide and position rgb/cct panels
  if(!enRGB)
  {
    lv_obj_add_flag(_panelRGB, LV_OBJ_FLAG_HIDDEN);
    lv_obj_align(_panelCCT, LV_ALIGN_CENTER, 0, -25);
  }
  if (!enCCT)
  {
    lv_obj_add_flag(_panelCCT, LV_OBJ_FLAG_HIDDEN);
    lv_obj_align(_panelRGB, LV_ALIGN_CENTER, 0, -25);
  }
}
